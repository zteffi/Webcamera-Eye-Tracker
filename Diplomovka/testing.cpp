...
int main(int, char) {
	...
	for (float gamma = .1; gamma< 2; gamma *= 2) {
		unsigned long frameCount = 0;
		ofstream csv;
		csv.open("..\\test_results\\us_" + to_string(gamma) + "_gamma_.csv");
		csv << "test #1" << ";\n";
		...
		while (true) {
			...
			Point leftCenter = ip.getEyeCenter(red, leftEye, 200, gamma);
			GetSystemTime(&time);
			toc = (time.wSecond * 1000) + time.wMilliseconds;
			Point rightCenter = ip.getEyeCenter(red, rightEye, 200, gamma);
